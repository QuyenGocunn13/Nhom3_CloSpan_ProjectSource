                    ...
                    if minsup <= 0:
                        ...
                        if not all(col in df.columns for col in required_columns):
                            ...
                            if df['timestamp'].isnull().any():
                                ...
                                if unique_users == 1:
                                    print("Phát hiện chỉ có 1 người dùng — phân tích thói quen cá nhân...")
                                    df = df.sort_values(by='timestamp')
                                    sequence = [[product] for product in df['product_name'].tolist()]

                                    def extract_purchase_sequences(sequence, max_len=3):
                                        results = []
                                        n = len(sequence)
                                        flat_seq = [item[0] for item in sequence]
                                        for l in range(2, max_len + 1):
                                            for i in range(n - l + 1):
                                                subseq = tuple(flat_seq[i:i + l])
                                                results.append(subseq)
                                        return results

                                    patterns_counter = Counter(extract_purchase_sequences(sequence))
                                    patterns = {
                                        tuple([ (item,) for item in pattern ]): count
                                        for pattern, count in patterns_counter.items()
                                    }
                                    print(f"Tìm thấy {len(patterns)} mẫu lặp lại trong thói quen cá nhân.")
